...
export default function Home() {
  ...
  const customTip = (e: React.ChangeEvent<HTMLInputElement> ) => {
    ...
    const tipFraction = customTipValue / 100; // Convert percentage to decimal fraction
    setCustomAmount(customTipValue); // Optional: If you need to keep track of the custom tip value separately
    setTip(tipFraction);
    setIsSelected(false);
    setIsCustom(true);
  }
  ...
  const tipClass: string = 'px-5 py-2.5 bg-darkCyan rounded-md focus:bg-cyan focus:text-darkCyan tipHover';
  const pickedTipClass: string = 'px-5 py-2.5 rounded-md bg-cyan text-darkCyan tipHover';
  ...
  return (
    <div className="background flex min-h-screen flex-col items-center font-space-mono">
      ...
      <div className="card grid grid-cols-2 mt-10 p-7 gap-9 bg-white rounded-3xl ">
        <div className="col-span-2 md:col-span-1 column py-3 ps-3 pe-2">

          {/* Bill and Input */}
          <div>
            <p className="text-gray mb-1 text-sm">Bill</p>
            <div className=" bg-offWhite px-4 py-1 rounded-md flex flex-row justify-between items-center border-2 border-gray/0 focus-within:border-cyan">
              ...
              <input type="number" value={bill ?? ''} onChange={handleBillAmount} className="bg-offWhite text-2xl text-end border-0 text-darkCyan" min='0' step='0.01' placeholder="0"
              />
          ...
          <div className="mt-10 ">
            ...
            <div className="grid grid-cols-3 gap-3 text-xl text-offWhite">
              ...
              <input onChange={customTip} type="number" placeholder="Custom" className={!isCustom ? "py-2 bg-offWhite text-gray rounded-lg focus:text-darkCyan text-center cursor-pointer" : "py-2 bg-offWhite text-gray rounded-lg focus:text-darkCyan text-center custom"} />
